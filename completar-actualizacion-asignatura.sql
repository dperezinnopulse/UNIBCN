...
PRINT 'Completando actualizacion del dominio Asignatura (ID=12)...';
...
-- Verificar que no hay valores existentes
IF EXISTS (SELECT 1 FROM ValoresDominio WHERE DominioId = 12)
BEGIN
    PRINT 'ADVERTENCIA: Ya existen valores en el dominio Asignatura. Eliminando...';
    DELETE FROM ValoresDominio WHERE DominioId = 12;
    PRINT 'Valores existentes eliminados.';
END
