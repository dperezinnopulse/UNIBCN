...
const __apiBase = (function(){
  try {
    // Intentar usar meta tag primero
...
async function aplicarDatosReales(actividad) {
    Utils.log('*** FUNCION aplicarDatosReales EJECUTANDOSE ***');
    Utils.log('DEBUG: INICIO aplicarDatosReales - funcion ejecutandose');
    Utils.log('Aplicando datos reales al formulario...');
    
    // DEBUG: Mostrar TODOS los campos que devuelve el backend
    Utils.log('=== DEBUG: CAMPOS DEL BACKEND ===');
    Utils.log('Campos disponibles en actividad:', Object.keys(actividad));
    Utils.log('Valores de campos nuevos:');
    Utils.log('- metodologia:', actividad.metodologia);
    Utils.log('- sistemaEvaluacion:', actividad.sistemaEvaluacion);
    Utils.log('- horarioYCalendario:', actividad.horarioYCalendario);
    Utils.log('- observaciones:', actividad.observaciones);
    Utils.log('- espacioImparticion:', actividad.espacioImparticion);
    Utils.log('- lugarImparticion:', actividad.lugarImparticion);
    Utils.log('- costeEstimadoActividad:', actividad.costeEstimadoActividad);
    Utils.log('- estadoActividad:', actividad.estadoActividad);
    Utils.log('- remesa:', actividad.remesa);
    Utils.log('- tiposInscripcionId:', actividad.tiposInscripcionId);
    Utils.log('- fechaAdjudicacionPreinscripcion:', actividad.fechaAdjudicacionPreinscripcion);
    Utils.log('=== FIN DEBUG BACKEND ===');
    ...
    Utils.log('*** DEBUG: INICIANDO MAPEO DE CAMPOS ***');
    Utils.log(`DEBUG: Total campos en mapeo: ${Object.keys(mapeoCampos).length}`);
    Utils.log(`DEBUG: Campos en mapeo: ${Object.keys(mapeoCampos).join(', ')}`);
    
    // DEBUG ESPECÍFICO: Verificar si los campos nuevos están en el mapeo
    const camposNuevos = ['metodologia', 'sistemaEvaluacion', 'horarioYCalendario', 'observaciones', 'espacioImparticion', 'lugarImparticion', 'otrasUbicaciones', 'urlPlataformaVirtual', 'urlCuestionarioSatisfaccion', 'costeEstimadoActividad', 'estadoActividad', 'remesa', 'tiposInscripcionId', 'fechaAdjudicacionPreinscripcion'];
    Utils.log('*** DEBUG: VERIFICANDO CAMPOS NUEVOS EN MAPEO ***');
    camposNuevos.forEach(campo => {
        const estaEnMapeo = mapeoCampos.hasOwnProperty(campo);
        Utils.log(`DEBUG CAMPO NUEVO: ${campo} está en mapeo: ${estaEnMapeo ? 'SÍ' : 'NO'}`);
        if (estaEnMapeo) {
            Utils.log(`  -> Mapea a: ${mapeoCampos[campo]}`);
        }
    });
    Utils.log('*** FIN DEBUG CAMPOS NUEVOS ***');
    ...
    Object.keys(mapeoCampos).forEach(campoBackend => {
        ...
        // DEBUG: Log para campos nuevos
        if (['metodologia', 'sistemaEvaluacion', 'horarioYCalendario', 'observaciones', 'espacioImparticion', 'lugarImparticion', 'otrasUbicaciones', 'urlPlataformaVirtual', 'urlCuestionarioSatisfaccion', 'costeEstimadoActividad'].includes(campoBackend)) {
            Utils.log(`DEBUG CAMPO NUEVO: ${campoBackend} -> ${campoFormulario}, valor: "${valor}" (tipo: ${typeof valor})`);
        }
        ...
        if (valor !== undefined && valor !== null) {
            ...
            // DEBUG: Log para campos nuevos
            if (['metodologia', 'sistemaEvaluacion', 'horarioYCalendario', 'observaciones', 'espacioImparticion', 'lugarImparticion', 'otrasUbicaciones', 'urlPlataformaVirtual', 'urlCuestionarioSatisfaccion', 'costeEstimadoActividad'].includes(campoBackend)) {
                Utils.log(`DEBUG ELEMENTO: ${campoFormulario} existe: ${elemento ? 'SI' : 'NO'}`);
            }
